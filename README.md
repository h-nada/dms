...
| 日付 | バージョン | 内容 |
|------|----------|------|
| 2025-12-11 | 1.5.3 | QRコードリンク化・吹き出しモバイル対応 |
| 2025-12-11 | 1.5.2 | モバイル表示改善・GitHub Pages設定 |
| 2025-12-11 | 1.5.1 | ヒーローセクションのカラー統一・バッジサイズ調整 |
...
### v1.5.3 変更内容（2025-12-11）

**QRコードのリンク化（モバイル対応）**

- 各区のQRコード画像を`<a>`タグで囲み、タップでリンク先に遷移可能に
- 川崎区〜麻生区: 川崎市公式サイトの各区ページへリンク
- 全町連: https://kawa-zencho.com/ へリンク
- `.qr-link`クラスを追加（`display: block`）

**加入者の声セクション（`.voice-item__a-content`）吹き出しのモバイル対応**

- モバイルで吹き出しの三角形を上部中央に配置
- 人物画像が上、吹き出しが下の並びに対応
- `top: -15px` / `border-bottom-color` で上向き三角形を実現

---

### v1.5.2 変更内容（2025-12-11）

**GitHub Pages設定**

- リポジトリをパブリックに変更
- GitHub Pages を有効化（main ブランチ / root）
- 公開URL: https://h-nada.github.io/dms/

**モバイル表示の改善**

- `.about-section`: モバイルで背景画像を非表示に
- `.activity-grid__text`, `.voice-item__a-text`, `.contact-main__note`: 行末揃え（`text-align: justify`）
- `.hero__catchcopy`: HTMLから `<br class="sp">` を削除して2行表示に統一
- `.contact-main__left`: `justify-content: center` で中央揃え

---

### v1.5.1 変更内容（2025-12-11）

**ヒーローセクション（`.hero`）のカラー統一**

- `.hero__badge` の背景色を緑（`#2E7D32`）→ 紺色（`#00367f`）に変更
- `.hero__title` の文字色を紺色（`#00367f`）に統一
- `.highlight`、`.highlight-orange` の文字色を紺色（`#00367f`）に統一

**バッジサイズの拡大（`.hero__badge`）**

- フォントサイズ: PC `$font-size-sm` → `$font-size-lg` / SP `$font-size-xs` → `$font-size-md`
- パディング: PC `0.5rem 2rem` → `0.8rem 2.5rem` / SP `0.4rem 1.5rem` → `0.6rem 2rem`
